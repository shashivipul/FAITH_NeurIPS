...
def GDO_optimized(W):
    # Ensure W is a dense array
    if not isinstance(W, np.ndarray):
        W = W.toarray()

    # Get indices of the upper triangular matrix where W[i, j] > 0
    row, col = np.triu_indices(W.shape[0], k=1)
    mask = W[row, col] > 0
    row = row[mask]
    col = col[mask]
    weights = W[row, col]
    ...
    # Initialize the incidence matrix in COO format for efficiency
    data = []
    row_indices = []
    col_indices = []

    for m, (i, j, weight) in enumerate(zip(row, col, weights)):
        sqrt_weight = np.sqrt(weight)
        # Add contributions for edge (i, j)
        data.extend([sqrt_weight, -sqrt_weight])
        row_indices.extend([i, j])
        col_indices.extend([m, m])

    # Construct the incidence matrix
    delta = coo_matrix((data, (row_indices, col_indices)), shape=(W.shape[0], num_edges))
