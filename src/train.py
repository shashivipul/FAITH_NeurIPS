...
def main():
    ...
    if param['ablation_mode'] == 0: 
        ...
        out_t, _, test_teacher, _, _,node_energy_teacher,best_IF_t = train_teacher(param, model_t, g, feats, labels, indices, criterion_l, evaluator, optimizer_t)
        ...
        _, test_acc, test_val, test_best, _,best_IF_s = train_student(param, model_s, g, feats, labels, out_t, indices, criterion_l, criterion_t, evaluator, optimizer_s,node_energy_teacher,KD_model) 
        
        return test_teacher, test_acc, test_val, test_best,best_IF_t,best_IF_s
...
if __name__ == "__main__":
    ...
    if param['save_mode'] == 0:
        set_seed(param['seed'])
        test_teacher, test_acc, test_val, test_best,best_IF_t,best_IF_s = main()
        ...
        for seed in range(5):
            ...
            IF_list_teacher.append(best_IF_t)
            IF_list_student.append(best_IF_s)
    ...
    print('Teacher')
    print('Teacher accuracy',np.mean(test_teacher_list)*100, np.std(test_teacher_list)*100)
    print('Teacher IF',np.mean(IF_list_teacher),np.std(IF_list_teacher))
    print('Student')
    print('Student accuracy',np.mean(test_val_list)*100, np.std(test_val_list)*100)
    print('Student IF',np.mean(IF_list_student),np.std(IF_list_student))
